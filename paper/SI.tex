...
$R_{\mathrm{G}}^{0}$ (ungated resistance of PG), $\delta$ (as defined
in main text Section \ref{main-sec:res-1}). It is pointed out that the
scaling law for $R_{\mathrm{PCTE}}$ and $R_{\mathrm{G}}^{0}$
differs. For a long channel like PCTE, the resistance is derived from
Fick's law as\cite{O_Hern_2014}:
\begin{equation}
\label{eq:R-PCTE}
...
Next we show how the effective salt rejection ratio through PG, $\xi$,
...
$R_{\mathrm{PCTE}}$, the effective salt rejection through PG upon gating, is always larger than that expperimentally observed.
