...
\begin{abstract}
  {\normalsize
    Atomically thin porous graphene membranes are emerging as one of
...
controversial if the surface charges can result in a degree of salt
rejection, as the surface charge and potential cannot be precisely
quantified and are sensitive to the surroundings as well as the
history of chemical treatment \cite{Li_2008}. There remains a lack of
fundamental understanding of the interactions between the charged 2D
nanopores and the electrolyte solution. In this report, we investigate
electrolyte permeation through graphene nanopores that are
significantly larger than the hydrated radii. We construct the surface
...
% extract the
the effective degree of salt rejection from bare PG $\xi$ as a
function of $V_{\mathrm{G}}$ ( see Supplementary Section
\ref{SI-sec:R-model}) is given by:
\begin{equation}
\label{eq:xi-def}
...
notice that the above theoretical analysis suggests an upper limit of
$\xi$ of ∼0.97 (please confirm), even when
$\lambda_{\mathrm{D}}/r_{\mathrm{G}} \gg 1$, which appears to violate
the small pore limit when the pore size is smaller than the hydrated
radii. Indeed, our theory is derived from on the macroscopic transport
equations by treating ions as zero-volume charges, which may not be
valid when $R_{\mathrm{G}}$< 5 nm\cite{Jain_2015}. In addition, the
specific ion-graphene interactions \cite{Rollings_2016} are not taken
into account. Advanced theoretical frameworks that bridge continuum
and molecular scales are required to resolve the predict the
electrolyte transport when the pore size is sub-5 nanometers.
% Nevertheless
