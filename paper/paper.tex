...
\clearpage{}
\begin{abstract}
  {\normalsize Atomically thin porous graphene membranes are emerging as one
    of the most promising candidates as next-generation membrane
    technology owing to their unique transport physics and high permeation. Among the current challenges of 2D membranes
    is the understanding of fundamental ion transport as well as its control towards applications in ion rectification, ionic transistors or desalination. 
    Here we report the study of tailoring the ion transport 
    through atomically-thin porous graphene with pore sizes of
    20$\pm$10 nanometers, and a pore density of 1.25$\pm$0.25$\times$10$^{10}$
    cm$^{-2}$. By electrostatically gating a sheet of porous graphene, we demonstrate
    a considerable reduction of salt flux by up to 55\% and
    85\% using a gate voltage, based on our experiments and
    theoretical predictions, respectively. We systematically
    investigate the effects of salt concentration and species,
    including the development of a theory to model the electrolyte diffusion
    ...
    observed salt flux reduction, when the pore size is comparable to the
...
with those of bare PCTE membrane, suggesting the transport resistance
of PCTE $R_{\mathrm{PCTE}}$, is dominant over that of PG,
$R_{\mathrm{G}}^{0}$. Using a simple circuit model, we estimate the
ratio $\delta$ between $R_{\mathrm{G}}^{0}$ and $R_{\mathrm{PG}}$,
...
\todo[inline]{Make sure that the values used are correct.}
