...
\clearpage{}
\begin{abstract}
  {\normalsize Atomically thin porous graphene membranes are emerging
...
Section \ref{SI-sec:exp}).  The system presented here allows us to systematically
...
$\delta = R_{\mathrm{PCTE}}/R_{\mathrm{G}}^{0}$, to be ~2, which is in
good agreement with the value derived from the experimental data in
\Fig{} \ref{fig:2}b (details see Supplementary Section
\ref{SI-sec:R-model}).
\todo[inline]{Make sure that the values used
  are correct.}
...
solution (Supplementary Section \ref{SI-sec:copper}). The slope corresponding to the
...
PG-PCTE is reduced upon gating (i.e. salt penetration through the PG
...
$V_{\mathrm{G}}$ as (details see Supplementary Section
\ref{SI-sec:R-model}):
\begin{equation}
\label{eq:xi-def}
...
10$^{-2}$ M KCl (see Supplementary Section \ref{SI-sec:debye}) exhibit a power law
...
Supplementary Section \ref{SI-fig:scheme}). Indeed, the interplay between graphene
...
nanopore studies \cite{Rollings_2016}.
...
(\Fig{} \ref{fig:4}d). A total flux reduction of up to 80\% is
...
\ref{SI-sec:simu-res}). Intriguingly, by increasing $\psi_{\mathrm{G}}$
...
concentration near the pore edge (prefered path) greatly increases
(details see Supplementary Section \ref{SI-sec:simu-res}, Supplementary
